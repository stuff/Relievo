...
- **Alternate elements get dedicated props**, not `render`. For example, `Button` takes `href`.
  - **`as`** picks the HTML element among a **closed list** of elements that behave the same and only change the semantics, such as `Card` (`div` / `article` / `section`) or `Card.Title` (`h2`–`h6`). The look never changes with `as`. Anything that changes behaviour (a link, a button) gets its own prop instead.
...
- **Floating: a layer above the page.** A select's list (`--ui-shadow-floating`, surface background, border). Its content is flat: the options are rows, not buttons.
